...
gcom_args = {'window':3600, 'cyclecount':100}
gcom_args['target'] = {'data':job['data']['gfiles'], 'notifications':job['notifications']}
gcom_args['ledger'] = {'node':job['data']['gledger']['location']['node'],'name':'gcom_status'}

oceansfiltdef = []
oceansfiltdef.append({'filt':'substring','target':'name','startswith':'AMSR2-OCEAN','name':'AMSR2 Ocean Test'})

job['sources']['satepsanone']['paths']['gcom'] = {'path':'/pub/product/nde/amsr2/L2', 'dirs':{}, 'special':{}}
job['sources']['satepsanone']['paths']['gcom']['special'] = {'module':'satepsanone_puller','class':'FilePuller','args':gcom_args, 'filt':oceansfiltdef}
